...
def convert_to_ilp(nodes, edges):
    ...
    for i in range(len(variables)):
        ...
        constr = pulp.LpConstraint(
            lhs, pulp.LpConstraintEQ, f"simplex constraint node {i}", 1
        )
        ilp += constr
    ...
    for i in range(len(edge_variables)):
        obj = pulp.LpAffineExpression(
            [(edge_variables[i][j], edges[i].costs[j]) for j in range(len(edge_variables[i]))]
        )
        ilp += obj
